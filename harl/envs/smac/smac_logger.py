...
class SMACLogger(BaseLogger):
    ...
    def episode_log(
        ...
        self.writter.add_scalar(
            "incre_win_rate", incre_win_rate, self.total_num_steps
        )
        ...
        max_episode_len = (
            np.max(self.episode_lens) if len(self.episode_lens) > 0 else 0.0
        )
        min_episode_len = (
            np.min(self.episode_lens) if len(self.episode_lens) > 0 else 0.0
        )
        ...
        avg_reward_train = average_episode_len * critic_train_info["average_step_rewards"]
        ...
        self.writter.add_scalar("train_episode_rewards", avg_reward_train, self.total_num_steps)
