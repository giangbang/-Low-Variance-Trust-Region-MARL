...
class OnPolicyHARunner(OnPolicyBaseRunner):
    ...
    def train(self):
        ...
        if self.share_param and hasattr(self.actor[0], "share_param_train"):
            actor_train_infos = self.actor[0].share_param_train(
                self.actor_buffer, advantages.copy(), self.num_agents, self.state_type, agent_order
            )
        else:
            ...
            for i, agent_id in enumerate(agent_order):
                ...
                # )
                old_actions_logprob = check(self.actor_buffer[agent_id]
                                                .action_log_probs
                                                .reshape(-1, self.actor_buffer[agent_id]
