...
class OnPolicyBaseRunner:
    ...
    def __init__(self, args, algo_args, env_args, trial: "optuna.Trial"=None):
        ...
        self.share_param = bool(algo_args["algo"]["share_param"])
        self.fixed_order = bool(algo_args["algo"]["fixed_order"])
        ...
        if not self.algo_args["render"]["use_render"]:  # train, not render
            ...
            save_config(args, algo_args, env_args, self.run_dir, self.writter)
        ...
        if self.fixed_order:
            print("Fixing agents' order during training.")
        ...
        for episode in range(1, episodes + 1):
            ...
            if episode % self.algo_args["train"]["eval_interval"] == 0:
                if self.algo_args["eval"]["use_eval"]:
                    self.prep_rollout()
                    eval_avg_rew = self.eval()
                    if self.trial is not None:
                        self.trial.report(eval_avg_rew, self.logger.total_num_steps)
