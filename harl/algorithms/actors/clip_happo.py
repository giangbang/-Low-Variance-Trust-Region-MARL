...
from harl.algorithms.actors.ppo_share_param_update import share_param_train


class ClipHAPPO(OnPolicyBase):
    def __init__(self, args, obs_space, act_space, device=torch.device("cpu")):
        """Initialize clip-HAPPO algorithm.
        Args:
            args: (dict) arguments.
    ...
    def update(self, sample):
        ...
        surr2 = (
            torch.clamp(imp_weights, 
                1.0 - self.clip_param, 
                1.0 + self.clip_param)
            * adv_targ
        )

        # clipping the $\Re^{1:m-1}$ factor at 1
        factor_batch = torch.clamp(factor_batch, max=1)
        ...
        if self.use_policy_active_masks:
            ...
            with torch.no_grad():
                estimate_adv = torch.sum(
                    factor_batch * adv_targ, dim=-1, keepdim=True
                )[active_masks_batch.bool()]
                info["adv_variance"] = torch.var(estimate_adv).item()
        else:
        ...
        total_loss = (policy_loss - dist_entropy * self.entropy_coef)  # add entropy term
        total_loss.backward()
        ...
        return policy_loss, dist_entropy, actor_grad_norm, imp_weights, info
    ...
    def share_param_train(self,
        ...
        return share_param_train(self, actor_buffer, advantages, \
                                num_agents, state_type, agent_order)
