...
class ClipHATRPO(OnPolicyBase):
    ...
    def update(self, sample):
        ...
        return kl, loss_improve, expected_improve, dist_entropy, ratio, info
    ...
    def train(self, actor_buffer, advantages, state_type):
        ...
        for sample in data_generator:
            num_train += 1
            kl, loss_improve, expected_improve, dist_entropy, imp_weights, info = self.update(
                sample
            )
            ...
            train_info["n_backtrack"] += self.cnt_backtrack
            self.n_backtrack += self.cnt_backtrack

        self.n_backtrack /= num_train

        for k in train_info.keys():
            train_info[k] /= num_train
